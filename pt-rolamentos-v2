...
def download_and_load_model():
    """
    Baixa o modelo do Google Drive (se ainda não existir) e o carrega.
    ...
    if not os.path.exists(MODEL_FILENAME):
        try:
            ...
            st.info("Verifique se o ID do arquivo está correto e se o arquivo está público no Google Drive.")
            return None
    ...
    try:
        ...
        st.error(f"Erro ao carregar o modelo do arquivo '{MODEL_FILENAME}': {e}")
        st.warning("Isso pode ser causado por uma incompatibilidade de versões entre as bibliotecas (scikit-learn, numpy, etc.).")
        return None

# --- Carregar o modelo assim que o app iniciar ---
model = download_and_load_model()
if model is None:
    st.warning("O aplicativo não pode ser executado sem o modelo. Por favor, resolva os erros acima.")
    st.stop()
    ...
    if uploaded_file is None:
        ...
        try:
            with st.spinner("Lendo o arquivo de dados e executando a predição..."):
                ...
                if bd1.shape[1] < 28:
                    st.error(f"O arquivo carregado tem {bd1.shape[1]} colunas. São esperadas 28 colunas de dados.")
                    st.stop()
